    ...
    with torch.no_grad():
        ...
        if args.images_path:
            ...
            if args.images_path.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                ####kwargs
                if args.save_masks:
                    kwargs["save_masks_path"] = os.path.join(args.save_dir, "masks_processed/" +
                                                             args.images_path.split("/")[-1].split(".")[
                                                                 0] + "_masks.png")
                if args.save_depth_map:
                    kwargs["save_depth_map_path"] = os.path.join(args.save_dir, "depth_maps_processed/" +
                                                                 args.images_path.split("/")[-1].split(".")[
                                                                     0] + "_depth_map.png")
        ...
        if args.video_path:
            ...
            if args.video_path.lower().endswith(('.mp4', '.avi')):
                ...
                current_video_writer = cv2.VideoWriter(save_video_path, cv2.VideoWriter_fourcc(*'mp4v'),
                                                       current_video_cap.get(5), (im_r.shape[1], im_r.shape[0]))
                duration_of_record_frames = args.duration_of_record_sec * current_video_cap.get(
                    5) + args.skip_the_fist_frames
                while (ret):
